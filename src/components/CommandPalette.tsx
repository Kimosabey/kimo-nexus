...
export function CommandPalette({
  ...
  return (
    <div
      ...
      data-lenis-prevent
      // Ask mode has no list to arrow through, so Escape is handled at the dialog level.
      onKeyDown={(e) => {
        if (e.key === "Escape") {
          e.preventDefault();
          onClose();
      ...
      style={{ position: "fixed", inset: 0, zIndex: 150, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "12vh 20px 20px" }}
    >
      ...
      <div style={{ position: "relative", width: mode === "ask" ? "min(620px,100%)" : "min(560px,100%)", border: "1px solid var(--hairline)", borderRadius: 18, background: "var(--surface)", boxShadow: "var(--shadow-card)", overflow: "hidden", display: "flex", flexDirection: "column", minHeight: 0 }}>
        {/* mode switch */}
        <div role="tablist" aria-label="Palette mode" style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 12px", borderBottom: "1px solid var(--hairline)" }}>
          <button type="button" role="tab" aria-selected={mode === "jump"} onClick={() => setMode("jump")} style={tabBtn(mode === "jump")}>
            <Search size={14} strokeWidth={1.9} />
            {assistant.tabs.jump}
          </button>
          <button type="button" role="tab" aria-selected={mode === "ask"} onClick={() => setMode("ask")} style={tabBtn(mode === "ask")}>
            <Sparkles size={14} strokeWidth={1.9} />
            {assistant.tabs.ask}
          </button>
          <span style={{ marginLeft: "auto", fontFamily: "var(--fm)", fontSize: 10.5, color: "var(--muted)", border: "1px solid var(--hairline)", borderRadius: 6, padding: "3px 7px" }}>ESC</span>
        </div>
